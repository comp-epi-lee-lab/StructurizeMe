...
def load_data_hm450k(nrows):
    data = pd.read_csv('StructurizeMe/data/HM450k_EpicGeneSum1.tsv.gz', nrows=nrows, sep='\t', compression='gzip')
    return data

data = load_data_hm450k(22847)

def load_data_epic850k(nrows):
    df = pd.read_csv('StructurizeMe/data/EPIC850k_EpicGeneSum1.tsv.gz', nrows=nrows, sep='\t', compression='gzip')
    return df

df = load_data_epic850k(23747)

#def load_data_diff(nrows):
    #diff = pd.read_csv('StructurizeMe/data/Differences_HM450EPICv1.gz', nrows=nrows, sep='\t', compression='gzip')
    #return df
